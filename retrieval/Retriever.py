...
def _embed_query(query: str) -> List[float]:
    ...
    try:
        resp = ollama.embeddings(
            model=EMBED_MODEL, prompt=prefixed_query, options={"timeout": 20}
        )
...
def retrieve(query: str, top_k: int = TOP_K_CANDIDATES) -> List[Dict]:
    ...
    logger.info(
        "BM25 reranking: selected top %d from %d candidates",
        len(top_reranked),
        len(candidates),
    )
    ...
    for chunk in top_reranked:
        ...
        if running_token_count + chunk_tokens > TOKEN_BUDGET:
            logger.info(
                "Token budget reached: %d tokens (would exceed %d)",
                running_token_count,
                TOKEN_BUDGET,
            )
            break
    ...
    logger.info(
        "Final retrieval: %d chunks, %d tokens (budget: %d)",
        len(final_chunks),
        running_token_count,
        TOKEN_BUDGET,
    )
