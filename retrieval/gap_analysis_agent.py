...
def _parse_response(response: str) -> GapAnalysisResult:
    ...
    class_match = re.search(
        r"CLASSIFICATION:\s*(sufficient|partial|insufficient)", response, re.IGNORECASE
    )
    if class_match:
...
Analyze whether these passages are sufficient to answer the user's question."""
    ...
    try:
        response = ollama.chat(
            ...
            options={"temperature": LLM_TEMPERATURE, "timeout": 30},
        )
        ...
        logger.info(
            ...
            result.confidence,
            (
                f", refined query: {result.refined_query[:50]}..."
        ...
        return GapAnalysisResult(
            classification="insufficient", confidence=0.0, refined_query=query
        )


def run_gap_analysis(
    query: str, retriever_fn: Callable[[str], List[Dict]]
) -> GapAnalysisOutput:
    """
    ...
    for iteration in range(1, GAP_MAX_ITERATIONS + 1):
        ...
        logger.info(
            "Retrieved %d chunks for query: %.50s...", len(new_chunks), current_query
        )
        ...
        logger.info(
            "Added %d new chunks (total: %d)", added_count, len(accumulated_chunks)
        )
        ...
        if result.confidence >= GAP_CONFIDENCE_THRESHOLD:
            logger.info(
                "Gap analysis PASS: confidence %.2f >= %.2f after %d iterations",
                result.confidence,
                GAP_CONFIDENCE_THRESHOLD,
                iteration,
            )
        ...
        if iteration < GAP_MAX_ITERATIONS:
            ...
            if result.refined_query:
                ...
                logger.info(
                    "Refining query for next iteration: %.50s...", current_query
                )
            else:
    ...
    missing_info = (
        "Unable to find sufficient information after %d iterations. "
        % GAP_MAX_ITERATIONS
    )
